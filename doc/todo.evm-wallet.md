...
Created: 2026-09-05. Updated: 2026-09-06. Status: implementation and qualification
...
- [ ] Scaffold `apps/evm_wallet` with its own tile, resident tools, managed
...
- [ ] Define durable roots for accounts, configured networks/assets, commands,
...
- [ ] Implement the EVM RPC canister adapter through existing backend-call
...
- [ ] Add account/network selection, receive address, native/ERC20 balances,
...
- [ ] Prepare transactions with live balance, allowance, nonce, fee, and
...
- [ ] Build the durable command journal: authenticated caller installation plus
...
- [ ] Implement pending/unknown recovery, receipt status and finality, and
...
- [ ] Handle Arbitrum fee estimation and finality separately from Ethereum;
...
- [ ] Expose versioned closed-schema accounts, networks, balances, contract
...
- [ ] Test concurrent callers/tiles, separate chains, nonce collisions, changed
...
- [ ] Show the EVM Wallet confirmation and real operation/transaction evidence.
...
- [ ] Add browser/integration tests for decline, identical replay, changed
...
- [ ] Fix false mint completion: replace the current “balance increased” check
...
- [ ] Persist deposit intent, source network/account, helper/token, IC recipient,
...
- [ ] Fix receipt-timeout retry behavior: retain pending/unknown operations,
...
- [ ] Repair older IC Send/Withdraw recovery gaps with durable commands and
...
- [ ] Handle each ledger/minter's actual reconciliation semantics, including
...
- [ ] Add regression tests for an accepted transfer with a lost reply,
...
- [ ] Add EVM Wallet as a deposit source while retaining existing external
...
- [ ] Keep bridge orchestration in IC Wallet: validate helper/minter identity,
...
- [ ] Support redemption to an EVM Wallet address through the existing minter
...
- [ ] Add versioned bridge/quote/status tools as needed for the workflow. Agent
...
- [ ] Preserve released `wallet` v1 and `wallet_commands` v1 schema sources and
...
- [ ] Scaffold `apps/uniswap` as a separate app with its own swap UI, resident
...
- [ ] Implement account/network selection, token pair and amount, balances,
...
- [ ] Build exact approval/permit and swap requests from the selected quote.
...
- [ ] Implement the approval model required by supported routes: ordinary ERC20
...
- [ ] Request signatures and transactions only through EVM Wallet. Uniswap owns
...
- [ ] Persist quote-derived intent, approval/permit progress, wallet request IDs,
  ...
  sequence is atomic.
- [ ] Expose versioned read/quote/prepare/status tools for Agent. In Agent Mode,
...
- [ ] Handle declined review, insufficient token/gas, fee/quote changes,
  slippage revert, expired permits, pending/replaced transactions, provider
  disagreement, and missing/incompatible EVM Wallet.
- [ ] Test Ethereum and Arbitrum swaps using reproducible fork/test fixtures:
...
- [ ] Test old/new Kernel and app combinations: existing IC Wallet remains
...
- [ ] Audit every changed app's managed roots. Prove clean initialization,
...
- [ ] Increase each changed production app's release version only after its
...
- [ ] Publish the compatible Kernel and app set in one catalog transaction
...
- [ ] Repeat `npm run updates:publish` with identical bytes and require receipt
...
- [ ] Verify state-preserving in-product upgrades. Leave Dispenser starter
