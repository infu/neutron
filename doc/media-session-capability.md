...
Status: Kernel 0.3.13 implementation candidate
Target: a generic, owner-approved ephemeral media surface for declaring apps
...
The Kernel does not provide signaling, peer discovery, rooms, TURN, or meeting
state. A declaring application remains responsible for its media protocol and
for bounding, authenticating, expiring, and minimizing any signaling records.
WebRTC media is encrypted in transit, but ICE may disclose peer network
addresses and a TURN relay can observe connection metadata. Applications must
describe those limits accurately and distinguish direct from relayed paths.
...
This is the concrete change surface for the Kernel-brokered ephemeral
nonce-origin media iframe. The capability does not authorize ambient tile media
access or a weaker fallback.
...
8. Validate at least one declaring example app with two-browser fake-media
   tests, including application-owned signaling and direct/relayed status.

No declaring app should ship the capability until steps 1–7 pass against the
exact Kernel release candidate and its own media protocol is tested separately.
