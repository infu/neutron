...
module {
  ...
  public class Digest() {
    ...
    private let s = Array.toVarArray<Nat32>(S);

    private let x = VarArray.repeat<Nat8>(0, 64);
    ...
    public func reset() {
      for (i in Nat.rangeInclusive(0, 7)) {
        s[i] := S[i];
      };
      nx := 0;
      len := 0;
    ...
    public func write(data : [Nat8]) {
      ...
      if (nx > 0) {
        ...
        for (i in Nat.rangeInclusive(0, n - 1)) {
          x[nx + i] := p[i];
        };
        nx += n;
        if (nx == 64) {
          let buf = Array.fromVarArray<Nat8>(x);
          block(buf);
          nx := 0;
        };
      ...
      if (p.size() > 0) {
        for (i in Nat.rangeInclusive(0, p.size() - 1)) {
          x[i] := p[i];
        };
        nx := p.size();
      };
    ...
    public func sum() : [Nat8] {
      ...
      n := n << 3;
      buf := VarArray.repeat<Nat8>(0, m);
      if (m > 0) {
        buf[0] := 0x80;
      };
      write(Array.fromVarArray<Nat8>(buf));
      buf := VarArray.repeat<Nat8>(0, 8);
      for (i in Nat.rangeInclusive(0, 7)) {
        let j : Nat64 = 56 -% 8 *% Nat64.fromIntWrap(i);
        buf[i] := Nat8.fromIntWrap(Nat64.toNat(n >> j));
      };
      write(Array.fromVarArray<Nat8>(buf));
      let hash = VarArray.repeat<Nat8>(0, 32);
      for (i in Nat.rangeInclusive(0, 7)) {
        for (j in Nat.rangeInclusive(0, 3)) {
          let k : Nat32 = 24 -% 8 *% Nat32.fromIntWrap(j);
          hash[4 * i + j] := Nat8.fromIntWrap(Nat32.toNat(s[i] >> k));
        };
      };
      let hash224 = VarArray.repeat<Nat8>(0, 28);
      for (i in Nat.rangeInclusive(0, 27)) {
        hash224[i] := hash[i];
      };
      return Array.fromVarArray<Nat8>(hash224);
    };

    private func block(data : [Nat8]) {
      var p = data;
      var w = VarArray.repeat<Nat32>(0, 64);
      while (p.size() >= 64) {
        var j = 0;
        ...
        var g = s[6];
        var h = s[7];
        for (i in Nat.rangeInclusive(0, 63)) {
