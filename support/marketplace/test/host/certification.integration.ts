...
import { installFixture, session, type IntegrationCase, type Fixture } from "./helpers.ts";
...
async function certifiedHttp(pic: any, fixture: Fixture) {
  const { actor, canisterId } = fixture;
  ...
  return { read, rejectsStaleWitness };
}
...
export const cases: IntegrationCase[] = [{
  ...
  async run() {
    ...
    try {
      ...
      const { read, rejectsStaleWitness } = await certifiedHttp(pic, fixture);
      ...
      assert.deepEqual(names, ["approveSuccessorAndRetire", "buyerOwnershipRetained", "http_request", "http_streaming_callback", "initialize", "removeAuditor", "revokeCandidate", "revokeGrant", "setFree"].sort());
  ...
  name: "Production certification retires replaced artifact bytes and proofs through upgrade",
  scope: "http",
  async run() {
    const { pic, shutdown } = await session();
    try {
      ...
      const [packagePath, sourcePath, imagePath] = await actor.initialize();
      const { read, rejectsStaleWitness } = await certifiedHttp(pic, fixture);
      const packageRequest = request(packagePath, tokens.buyer);
      const sourceRequest = request(sourcePath, tokens.buyer);
      const priorPackage = await read(packageRequest, 200);
      const priorSource = await read(sourceRequest, 200);
      assert.ok(priorPackage.continuation);
      assert.deepEqual(priorPackage.assembled.body, Buffer.concat([Buffer.alloc(1_048_576, 65), Buffer.alloc(17, 66)]));
      assert.equal(priorSource.assembled.body.toString(), "private-offered-source");
      await read(request(imagePath), 200);

      // The fixture calls the real retention deletion and removes certification
      // paths in the same update that installs the new approved candidate.
      const result = await actor.approveSuccessorAndRetire();
      assert.equal(result.retiredCount, 2n);
      assert.equal(result.retiredMissing, true);
      assert.equal(result.historyRetained, true);
      assert.equal(await actor.buyerOwnershipRetained(), true);
      const [successorPackage, successorSource, retainedImage] = result.paths;
      assert.equal(retainedImage, imagePath);
      assert.notEqual(successorPackage, packagePath);
      assert.notEqual(successorSource, sourcePath);
      const absentPackage = await read(packageRequest, 404);
      const absentSource = await read(sourceRequest, 404);
      await rejectsStaleWitness(packageRequest, priorPackage.assembled, absentPackage.assembled);
      await rejectsStaleWitness(sourceRequest, priorSource.assembled, absentSource.assembled);
      await assert.rejects(() => actor.http_streaming_callback(priorPackage.continuation));

      // Every artifact was configured at all canonical route kinds. Leaving
      // even a denial leaf for a deleted digest prevents valid wildcard 404s.
      const retiredPaths = [packagePath, sourcePath].flatMap((path: string) => {
        const digest = path.match(/[0-9a-f]{64}/)?.[0];
        assert.ok(digest);
        return [`/repo/v1/packages/${digest}.neutron`, `/repo/v1/sources/${digest}.source.v1.msgpack.gz`, `/repo/v1/media/${digest}`];
      });
      for (const path of retiredPaths) {
        await read(request(path), 404);
        for (const token of Object.values(tokens)) await read(request(path, token), 404);
      }
      const successorToken = "4".padStart(64, "0");
      const checkRetained = async () => {
        assert.equal(await actor.buyerOwnershipRetained(), true);
        await read(request(successorPackage), 403);
        await read(request(successorPackage, tokens.buyer), 403);
        const currentPackage = await read(request(successorPackage, successorToken), 200);
        const currentSource = await read(request(successorSource, successorToken), 200);
        const image = await read(request(imagePath), 200);
        assert.equal(currentPackage.assembled.body.toString(), "successor-package");
        assert.equal(currentSource.assembled.body.toString(), "successor-offered-source");
        assert.equal(image.assembled.body.toString(), "public-listing-image");
      };
      await checkRetained();

      await pic.upgradeCanister({ canisterId, wasm: wasmPath, arg: new Uint8Array(), upgradeModeOptions: { skip_pre_upgrade: [], wasm_memory_persistence: [{ keep: null }] } });
      for (const path of retiredPaths) await read(request(path, tokens.buyer), 404);
      await assert.rejects(() => actor.http_streaming_callback(priorPackage.continuation));
      await checkRetained();
    } finally {
      await shutdown();
