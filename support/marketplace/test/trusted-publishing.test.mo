...
persistent actor {
  ...
  func memory(publisherMemory : PublisherStore.Mem) : Store.Mem {
    ...
    Store.setTrustedPublishingPrincipal(Store.Use(mem, publisherMemory, ReleaseStore.init()), ?trusted());
    mem;
  };
  ...
  public func trusted_cli_publishes_two_apps_and_exact_retry_survives_memory_rebind() : async Test.Metrics {
    Test.test(func () {
      ...
      let mem = memory(publisherMemory); let db = Store.Use(mem, publisherMemory, ReleaseStore.init());
      ...
      for (value in result.batch.entries.vals()) {
        ...
        assert ReleaseStore.heads(db.channels, app.appId).betaHead.candidateId == ?saved.id and Catalog.eligibleFor(db, app, #beta);
  ...
  public func trusted_successor_retires_old_bytes_and_keeps_batch_candidate_and_audit_history() : async Test.Metrics {
    Test.test(func () {
      ...
      assert ReleaseStore.heads(db.channels, appId).betaHead.candidateId == ?second.id;
      ...
      assert ReleaseStore.heads(restored.channels, appId).betaHead.candidateId == ?second.id;
