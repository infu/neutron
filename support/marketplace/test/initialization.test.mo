...
persistent actor {
  ...
  public func entire_inventory_is_validated_before_mutation() : async Test.Metrics {
    Test.test(func() {
      ...
      let db = Store.Use(Store.init(config()), publisherMemory, ReleaseStore.init());
  ...
  public func reservations_are_atomic_hidden_and_never_overwrite_existing_rows() : async Test.Metrics {
    Test.test(func() {
      ...
      assert Store.getApp(Store.Use(mem, publisherMemory, db.channels), original.appId) == ?first;
  ...
  public func omitted_reservations_keep_clean_initialization_compatible() : async Test.Metrics {
    Test.test(func() {
      let publisherMemory = PublisherStore.init();
      let db = Store.Use(Initialization.memory({ config() with reservations = null; trustedPublishingPrincipal = null }, 0, publisherMemory), publisherMemory, ReleaseStore.init());
