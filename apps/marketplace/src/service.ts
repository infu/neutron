...
async function uiRead(context: MsgBusToolContext, method: string, args: JsonObject): Promise<unknown> {
  ...
  switch (method) {
    case "storefront": return client.storefront(args as unknown as { search: string; tag?: string });
    case "catalog": return client.catalog(args as unknown as { tier: AppTier; window: RankingWindow; search: string; cursor?: string; tag?: string; exclude?: string[] });
...
register("marketplace_storefront_v1", "Marketplace categories and featured apps", "Read the editable category IDs and curated featured apps for the current release preference.", { search: string, tag: string }, [], reads, async (args, context) => (await protocolClient(context)).storefront({ search: text(args.search), ...(typeof args.tag === "string" ? { tag: args.tag } : {}) }));
register("marketplace_catalog_v1", "Discover marketplace apps", "Browse audited free or paid apps ranked by distinct acquisitions over rolling 7/30 days or all time. Filter by tag and exclude displayed featured app IDs. Queries go directly to the protocol. Follow nextCursor with the same filters for another page.", { tier: { enum: ["free", "paid"] }, window: { enum: ["week", "month", "all"] }, search: string, tag: string, exclude: { type: "array", items: string }, cursor: string }, [], reads, async (args, context) => (await protocolClient(context)).catalog({ tier: args.tier === "paid" ? "paid" : "free", window: args.window === "month" || args.window === "all" ? args.window : "week", search: text(args.search), exclude: Array.isArray(args.exclude) ? args.exclude as string[] : [], ...(typeof args.tag === "string" ? { tag: args.tag } : {}), ...(typeof args.cursor === "string" ? { cursor: args.cursor } : {}) }));
