...
export function AppDetailDialog({ client, app, close, acquire, install, connected, connect, installing = false, discount = noDiscount, publisher, refreshRevision = 0 }: {
  ...
  return <Modal title={shown.title} close={close} wide footer={<><span className="mp-muted mp-footer-note">{aheadOfStable ? "Ahead of stable — waiting for a stable release" : installed ? "Installed. Manage app updates in Settings." : shown.owned ? "Owned by this Neutron · Future updates included" : "One acquisition. All future approved updates."}</span>{shown.channel === "beta" && <span className="mp-badge mp-release-beta">Beta v{shown.version}</span>}{shown.owned || installed ? <InstallControl client={client} appIds={[shown.id]} selectionIdentity={selectionIdentity} disabled={installed} busy={installing} label={installed ? "Installed" : "Install app"} onInstall={install} className="mp-primary" /> : <button className="mp-primary" type="button" onClick={() => acquire(shown)}>{BigInt(shown.priceUsdMicros) === 0n ? "Get app" : `Get · ${usd(discountedPrice(shown.priceUsdMicros, discount))}`}</button>}</>}>
