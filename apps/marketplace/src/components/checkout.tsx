...
import type { AppListing, EthereumWalletSource, MarketplaceClient, OperationResult, PaymentToken, PurchaseQuote } from "../view-types.ts";
...
export function Checkout({ client, apps, close, complete, pending }: {
  ...
  useEffect(() => { if (!dispatched) setQuote(null); }, [token, code, source, wallet, dispatched]);
  ...
  async function review() {
    ...
    try {
      ...
      const reviewed = await client.quotePurchase({ appIds: apps.map((app) => app.id), token: source === "ethereum" ? "ckUSDC" : token, affiliateCode: code.trim(), ...(source === "ethereum" ? { ethereum: { wallet, ...(payerAddress ? { payerAddress } : {}) } } : {}) });
      if (mounted.current) { setQuote(reviewed); setKnownPaid(BigInt(reviewed.payment.atoms) > 0n); }
    }
