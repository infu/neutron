...
import { AppIcon, EmptyState, ErrorNote, Icon, Loading, acquisitionStats, dateLabel, errorMessage, usd, useRead } from "./components/primitives.tsx";
...
export default function App({ client: suppliedClient }: { client?: MarketplaceClient }) {
  ...
  return <main className="nt-app mp-app"><div className="mp-shell">
    ...
    <div className="mp-body">
      ...
      {tab === "library" && saved.data && saved.data.filter((item) => !item.installation && item.state !== "complete" && item.operationId !== active?.result.operationId).map((item) => <section className="mp-operation" key={item.operationId}><strong>Saved action</strong><p>{item.message}</p><button className="mp-secondary" type="button" onClick={() => setActive({ result: item, resume: () => resumeSaved(item) })}>View saved progress</button></section>)}
