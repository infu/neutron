...
function Explore({ client, refresh, discount, editDiscount, select }: { client: MarketplaceClient; refresh: number; discount: DiscountPreference; editDiscount: () => void; select: (app: AppListing) => void }) {
  ...
  return <div className="mp-explore">
    ...
    {(["paid", "free"] as const).map((tier) => <CatalogSection key={JSON.stringify([tier, window, search.trim(), refresh])} client={client} tier={tier} window={window} search={search.trim()} discount={discount} select={select} />)}
  </div>;
}

function CatalogSection({ client, tier, window, search, discount, select }: { client: MarketplaceClient; tier: AppTier; window: RankingWindow; search: string; discount: DiscountPreference; select: (app: AppListing) => void }) {
...
function AppCard({ app, discount, select }: { app: AppListing; discount: DiscountPreference; select: () => void }) {
  const acquisitions = acquisitionStats(app);
  return <button className="mp-app-card" type="button" onClick={select} aria-haspopup="dialog"><span className="mp-card-main"><AppIcon app={app} /><span className="mp-card-copy"><strong>{app.title}</strong><span className="mp-card-category">{app.category}</span><span className="mp-card-summary">{app.summary}</span></span></span><span className="mp-card-bottom"><span className="mp-card-metrics"><span className="mp-rating">{app.rating === null ? "New" : <>{app.rating.toFixed(1)} <span aria-hidden="true">★</span> <small>({app.ratingCount.toLocaleString()})</small></>}</span>{acquisitions && <span className="mp-acquisitions" title="All time">{acquisitions.count} {acquisitions.label}</span>}</span><span className="mp-card-price">{app.owned || app.installed || app.installedVersion ? "Owned" : <AppPrice micros={app.priceUsdMicros} discount={discount} />}</span></span></button>;
}
