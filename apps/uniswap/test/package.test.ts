...
const archive = async () => unpackNeutronPackage(await readFile(new URL("../uniswap.v0.1.20.neutron", import.meta.url)));
...
test("Uniswap owns its managed journal and declares exact public Wallet access", async () => {
  ...
  expect(value).toMatchObject({ id: "uniswap", version: 120, update_source: "sj2r4-haaaa-aaaay-aadgq-cai", memory: {
...
test("the packaged tile, resident service and managed-memory root are installable", async () => {
  ...
  expect(prepared.manifest.version).toBe(120);
...
test.each([
...
] as const)("release 120 preserves production %s backend and both root dependency closures", async (version, release, digest) => {
  ...
  expect(schema).toEqual({ ...priorSchema, app: { ...priorSchema.app, name: "via Uniswap", version: 120 } });
  const kernel = { format: 3 as const, id: "kernel", name: "Kernel", version: 100, entry: "f".repeat(64) };
