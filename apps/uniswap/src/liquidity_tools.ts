...
export function registerLiquidityTools() {
  ...
  exposeTool("uniswap_action_reconcile_v1", {
    ...
    description: "Refresh public transaction fields and receipts for hashes already linked to saved Wallet requests, following quote renewals. Updates only the Uniswap journal; never signs, broadcasts, opens a review, creates a quote or continues a queued step. Each step reports its exact requestId, whether it was checked live, and receipt status/block/finality. A disappeared receipt becomes pending/unknown; success is verified against saved sender, destination, calldata and value. A dispatched request with no saved hash stays unknown. A retained authorization denial/revocation can stop the workflow while execution remains unknown; neither proves an unsigned or rejected Wallet operation. Set includeAuthorization:true to return each step's saved authorizationFailure code/reason/requestId, or null if none was retained. Old records without this evidence remain unknown. Retrieve uniswap_action_input_v1 before explicit recovery, and do not replay a declined plan or successful mint.",
    inputSchema: toolSchema({ operationId: toolOperationId, includeAuthorization: { type: "boolean", description: "Include retained authorization errors separately from execution status. Default false preserves the original response shape." } }, ["operationId"]),
    outputSchema: toolSchema({ action: { oneOf: [toolSchema({ ...(actionOutputSchema.properties as JsonObject), steps: { type: "array", items: toolSchema({ label: toolText, kind: toolText, status: toolText, transactionHash: toolNullableText, requestId: toolText, checked: { type: "boolean" }, receipt: { oneOf: [toolSchema({ status: { enum: ["success", "reverted"] }, blockNumber: toolText, finality: { enum: ["included", "safe", "finalized"] } }), { type: "null" }] }, authorizationFailure: { oneOf: [toolSchema({ requestId: toolOperationId, code: { enum: ["AGENT_CONSENT_DENIED", "AGENT_MODE_REVOKED"] }, message: toolText }), { type: "null" }] } }, ["label", "kind", "status", "transactionHash", "requestId", "checked", "receipt"]) } }), { type: "null" }] } }),
    ...
    const result = await reconcileAction(createServiceWallet(context), createActionStore(context.kernel), String(args.operationId), {
      ...(context.signal ? { signal: context.signal } : {}), includeAuthorization: args.includeAuthorization === true, onProgress: (phase) => context.reportProgress({ phase }),
    });
