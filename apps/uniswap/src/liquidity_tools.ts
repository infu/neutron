...
export function registerLiquidityTools() {
  ...
  exposeTool("uniswap_action_reconcile_v1", {
    ...
    description: "Refresh linked hashes independently and save successful observations. Never signs, broadcasts, reviews or continues queued steps. Each step returns requestId, checked, and receipt outcome/block/finality. An archive/RPC error does not hide other steps: failed reads show read_unavailable, checked:false and receipt:null; prior journal evidence stays saved. includeDiagnostics:true adds readComplete and per-step readError. Read completeness is not financial completion. Missing hashes stay unknown; disappeared receipts become pending/unknown. Success requires matching saved sender, destination, calldata and value. A retained authorization denial/revocation may stop the workflow while execution stays unknown; it cannot prove an unsigned/rejected Wallet operation. includeAuthorization:true adds saved authorizationFailure code/reason/requestId or null. Retrieve uniswap_action_input_v1 before explicit recovery. Never replay a declined plan or successful mint.",
    inputSchema: toolSchema({ operationId: toolOperationId, includeAuthorization: { type: "boolean", description: "Include retained authorization errors separately from execution status. Default false preserves the original response shape." }, includeDiagnostics: { type: "boolean", description: "Include readComplete and per-step readError. Default false retains the original fields; errors still appear in the message and read_unavailable step status." } }, ["operationId"]),
    outputSchema: toolSchema({ action: { oneOf: [toolSchema({ ...(actionOutputSchema.properties as JsonObject), readComplete: { type: "boolean" }, steps: { type: "array", items: toolSchema({ label: toolText, kind: toolText, status: toolText, transactionHash: toolNullableText, requestId: toolText, checked: { type: "boolean" }, receipt: { oneOf: [toolSchema({ status: { enum: ["success", "reverted"] }, blockNumber: toolText, finality: { enum: ["included", "safe", "finalized"] } }), { type: "null" }] }, readError: { oneOf: [toolSchema({ code: toolNullableText, message: toolText }), { type: "null" }] }, authorizationFailure: { oneOf: [toolSchema({ requestId: toolOperationId, code: { enum: ["AGENT_CONSENT_DENIED", "AGENT_MODE_REVOKED"] }, message: toolText }), { type: "null" }] } }, ["label", "kind", "status", "transactionHash", "requestId", "checked", "receipt"]) } }, Object.keys(actionOutputSchema.properties as JsonObject)), { type: "null" }] } }),
    ...
    const result = await reconcileAction(createServiceWallet(context), createActionStore(context.kernel), String(args.operationId), {
      ...(context.signal ? { signal: context.signal } : {}), includeAuthorization: args.includeAuthorization === true, includeDiagnostics: args.includeDiagnostics === true, onProgress: (phase) => context.reportProgress({ phase }),
    });
    return toolJson({ action: result ? { ...compactActionResult(result), ...(result.readComplete === undefined ? {} : { readComplete: result.readComplete }), steps: result.steps } : null });
  });
