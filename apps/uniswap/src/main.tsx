...
export function App() {
  ...
  async function run(action: () => Promise<void>) { setBusy(true); setError(""); setProgress(""); setStartedAt(Date.now()); try { await action(); } catch (e) { setError(errorText(e)); } finally { setBusy(false); setProgress(""); setStartedAt(null); } }
  async function reload() { const page = await store.page(); setRecords(page.rows); setHistoryCursor(page.nextCursor); }
  async function loadOlder() { if (historyCursor === null) return; const page = await store.page(historyCursor); setRecords((previous) => [...new Map([...previous, ...page.rows].map((row) => [row.id, row])).values()]); setHistoryCursor(page.nextCursor); }
  ...
  async function refreshBalances(selected = account) {
    ...
    setProgress("Reading wallet balances…");
  ...
  async function getQuote() {
    ...
    setProgress("Connecting EVM Wallet…");
    await connectWalletReads();
    const next = await createIntent(wallet, { chainId: chain, accountId: account.accountId, accountAddress: getAddress(account.address), tokenIn, tokenOut, amountIn: amountAtoms(amount, tokenIn), slippageBps: bps, recipient: getAddress(recipient), deadline: (BigInt(Math.floor(Date.now() / 1000)) + BigInt(Math.round(duration * 60))).toString() }, setProgress);
    ...
    // Show the quote as soon as its route and allowance are known. Fee reads
    // remain visible work and cannot hide a successful quote behind more RPCs.
    setIntent(next);
    setProgress("Reading network fees…");
    const networkFees = await estimateSwapFees(wallet, next);
    if (currentDraft.current !== requestedDraft) throw new Error("Swap inputs changed while the quote was loading. Request a new quote.");
    setIntent({ ...next, quote: { ...next.quote, networkFees } });
  }
  async function save() { if (!intent) return; const record = await store.begin(intent); setIntent(null); setRecords((previous) => [record, ...previous.filter((row) => row.id !== record.id)]); await runStep(record, record.approval_request_id ? "approval" : "swap"); }
  ...
  async function refreshNetworkFees(record: SwapRecord) { await connectWalletReads(); setProgress("Reading network fees…"); const fees = await estimateSwapFees(wallet, savedIntent(record), approvalConfirmed(record)); setRefreshedFees((previous) => ({ ...previous, [record.id]: fees })); }
  return <main className="nt-app uni-app"><div className="uni-shell">
    ...
    {busy && <p role="status" className="uni-muted" data-testid="uniswap-progress">{progress || "Working…"}{startedAt !== null && ` · ${Math.max(0, Math.floor((now - startedAt) / 1000))}s`}</p>}
