...
type Props = { wallet: EvmWalletClient; account: EvmAccount | undefined; chain: Chain; tokens: Token[]; balance: (token: Token) => string; actions: ActionController; disabled: boolean; onTokens: (token: Token) => void };
type Editor = { operation: "mint" | "increase" | "decrease" | "collect" | "close"; position: PositionRecord | null };

export function LiquidityView({ wallet, account, chain, tokens, balance, actions, disabled, onTokens }: Props) {
  ...
  return <>
    {editor ? <section className="nt-panel uni-form uni-liquidity-editor" aria-label={title}>
      ...
      {["mint", "increase"].includes(editor.operation) && <>{[[selectedA, amountA, setAmountA, "A"], [selectedB, amountB, setAmountB, "B"]].map(([token, value, setter, side]) => { const current = token as Token; return <div className="uni-token-panel" key={String(side)}><label>Maximum deposit<input aria-label={`Token ${side} amount`} inputMode="decimal" placeholder="0" value={value as string} disabled={disabled || actions.busy} onChange={(event) => (setter as (value: string) => void)(event.target.value)}/></label><TokenPicker label={`Liquidity token ${side}`} tokens={available} value={keyOf(current)} disabled={disabled || actions.busy || !!position} onChange={(key) => side === "A" ? setTokenA(key) : setTokenB(key)}/><p className="uni-muted">Balance: {balance(current)} {current.symbol}</p></div>; })}<p className="uni-help">The pool uses the amounts it needs within your limits. Any unused tokens stay in your wallet.</p></>}
      ...
      {preview && <div className="uni-liquidity-preview"><p>{editor.operation === "collect" ? "Available to collect" : editor.operation === "decrease" ? "Estimated withdrawal" : "Estimated deposit"}</p><div><strong>{amountText(preview.amount0, preview.token0.decimals)} {preview.token0.symbol}</strong><strong>{amountText(preview.amount1, preview.token1.decimals)} {preview.token1.symbol}</strong></div></div>}
      ...
      {positions.map((position) => <article className="nt-panel uni-position-card" key={`${position.protocol}:${position.tokenId}`}>
        ...
        <div className="uni-position-tokens"><strong>{amountText(position.amount0, position.pool.token0.decimals)} <span>{position.pool.token0.symbol}</span></strong><strong>{amountText(position.amount1, position.pool.token1.decimals)} <span>{position.pool.token1.symbol}</span></strong></div>
        <p className="uni-collectible">Available to collect <span>{amountText(position.claimable0, position.pool.token0.decimals)} {position.pool.token0.symbol} + {amountText(position.claimable1, position.pool.token1.decimals)} {position.pool.token1.symbol}</span></p>
