...
type Position = { pool: VerifiedPool; lpBalance: string; assets: { token: Token; amount: string }[] };
...
function TokenPicker({ value, select, disabled }: { value: Token; select: (token: Token) => void; disabled: boolean }) {
  ...
  const balances = useRead(open && tokens.length ? `${value.chainId}:${tokens.map(tokenKey).join(",")}` : null, (signal) => wallet.balances({ accountId: "main", chainId: value.chainId, tokens: tokens.flatMap((token) => token.address ? [token.address] : []) }, { signal }));
  ...
  return <><button ref={trigger} type="button" className="cv-token-button" onClick={() => { setQuery(""); setOffset(0); setOpen(true); }} disabled={disabled} aria-label={`Select token, currently ${value.symbol}`}><TokenMark token={value} /><span>{value.symbol}</span><span aria-hidden="true">⌄</span></button>
    <dialog ref={dialog} className="cv-dialog" onCancel={() => setOpen(false)} onClose={() => setOpen(false)} aria-label="Select a token">
      ...
      <div className="cv-token-list">{tokens.map((token) => { const balance = balanceFor(balances.data, token); return <div key={tokenKey(token)} className="cv-token-row">
...
function AmountPanel({ token, value, change, output, balance, choose, disabled, label }: { token: Token; value: string; change?: (value: string) => void; output?: boolean; balance: string | null; choose?: (token: Token) => void; disabled: boolean; label: string }) {
  ...
  return <div className={`cv-amount ${output ? "cv-receive" : ""}`}><div className="cv-row cv-muted"><span>{label}</span>{balance !== null && <span>Balance: {display(balance, token.decimals, 5)}{!output && token.address && <button type="button" className="cv-text" onClick={() => change?.(formatUnits(BigInt(balance), token.decimals))} disabled={disabled}>Max</button>}</span>}</div>
    <div className="cv-amount-main">{output ? <output aria-label={label}>{value || "0"}</output> : <input aria-label={`${label} amount`} placeholder="0" inputMode="decimal" autoComplete="off" spellCheck={false} value={value} onChange={(event) => change?.(event.target.value)} disabled={disabled} />}
      {choose ? <TokenPicker value={asset} select={choose} disabled={disabled} /> : <span className="cv-token-fixed"><TokenMark token={asset} />{asset.symbol}</span>}</div>
...
function Settings({ slippage, setSlippage, recipient, setRecipient, disabled }: { slippage: string; setSlippage: (value: string) => void; recipient: string; setRecipient: (value: string) => void; disabled: boolean }) {
  return <details className="cv-details"><summary>Settings <span>{slippage || "—"}% slippage{recipient ? " · custom recipient" : ""}</span></summary><div className="cv-settings">
    ...
    <label>Receive at<input aria-label="Recipient address" placeholder="Your Ethereum Wallet" value={recipient} onChange={(event) => setRecipient(event.target.value)} disabled={disabled} /></label><p className="cv-muted">Leaving this blank sends assets to your wallet. The minimum received protects the transaction from price changes.</p>
  </div></details>;
}
...
function Preview({ plan, refreshing }: { plan: Plan; refreshing: boolean }) {
  ...
  return <div className="cv-preview"><div className="cv-row"><span className="cv-muted">Estimated network fee</span><span>{fees.loading ? "Estimating…" : total === null ? "Unavailable" : `${display(total, 18, 7)} ETH`}</span></div>{total !== null && <div className="cv-right"><Usd value={total} token={native(plan.chainId)} price={priceFor(native(plan.chainId))} /></div>}
    <details className="cv-details"><summary>Quote details <span>{refreshing ? "Refreshing…" : `${plan.steps.length} wallet ${plan.steps.length === 1 ? "step" : "steps"}`}</span></summary><div className="cv-settings"><dl>
      {plan.preview.outputs.map(({ token, minimum }) => <div key={tokenKey(token)}><dt>Minimum {token.symbol}</dt><dd>{display(minimum ?? "0", token.decimals, 10)}<Usd value={minimum ?? null} token={token} price={priceFor(token)} /></dd></div>)}
    ...
    </dl><p className="cv-muted">Quotes refresh automatically. Curve transactions enforce the minimum output and have no onchain expiry. Your wallet reviews the current amounts and fee before signing.</p>
    ...
    {plan.preview.warnings.length > 0 && <details><summary>Discovery observations</summary>{plan.preview.warnings.map((warning, index) => <p className="cv-muted" key={index}>{warning}</p>)}</details>}
    </div></details></div>;
}
...
function Swap({ chainId, balances, execute, busy, account, refresh }: { chainId: ChainId; balances: EvmBalancesResult | null; execute: (input: Input) => void; busy: boolean; account: EvmAccount | null; refresh: number }) {
  ...
  const selectedBalances = useRead(`${chainId}:${from.address}:${to.address}:${account?.address}`, (signal) => wallet.balances({ accountId: "main", chainId, tokens: [from, to].flatMap((token) => token.address ? [token.address] : []).filter((token, i, all) => all.indexOf(token) === i) }, { signal }), refresh + tick);
  ...
  const inputBalance = balanceFor(selectedBalances.data ?? balances, from);
  const insufficient = input?.kind === "swap" && inputBalance !== null && BigInt(input.amountIn) > BigInt(inputBalance);
  const output = preview.data?.preview.outputs[0];
  return <section className="cv-editor" aria-label="Swap"><div className="cv-editor-heading"><div><h2>Swap tokens</h2><p className="cv-muted">Trade through Curve with your Ethereum Wallet.</p></div><span className="cv-badge">{CHAINS[chainId].name}</span></div>
    <AmountPanel token={from} value={amount} change={setAmount} balance={inputBalance} choose={(token) => { if (tokenKey(token) === tokenKey(to)) setTo(from); setFrom(token); }} disabled={busy} label="You pay" />
    ...
    <AmountPanel token={to} value={output ? formatUnits(BigInt(output.amount), output.token.decimals) : ""} output balance={balanceFor(selectedBalances.data ?? balances, to)} choose={(token) => { if (tokenKey(token) === tokenKey(from)) setFrom(to); setTo(token); }} disabled={busy} label="You receive" />
    <Settings {...{ slippage, setSlippage, recipient, setRecipient }} disabled={busy} />
    <ErrorNote error={error || preview.error} />{preview.loading && !preview.data && <Spinner label="Finding your Curve quote…" />}
    {preview.data && <Preview plan={preview.data} refreshing={preview.loading} />}
    <button type="button" className="cv-primary" disabled={busy || !account || !input || !preview.data || preview.loading || !!preview.error || !!insufficient || !!error} onClick={() => input && execute(input)}>{!account ? "Connect Ethereum Wallet" : busy ? "Following wallet…" : insufficient ? `Insufficient ${from.symbol}` : !amount ? "Enter an amount" : preview.loading ? "Getting quote…" : "Review swap"}</button>
    <p className="cv-help">You approve each transaction in EVM Wallet.</p><ErrorNote error={selectedBalances.error} />
  </section>;
}

function PoolEditor({ pool, close, account, busy, execute, refresh }: { pool: VerifiedPool; close: () => void; account: EvmAccount | null; busy: boolean; execute: (input: Input) => void; refresh: number }) {
  ...
  const position = useRead(`${poolKey(pool)}:${account?.address}`, async (signal) => JSON.parse((await invoke<{ positionJson: string }>("curve_position_v1", { pool: refOf(pool) }, signal)).positionJson) as Position, refresh + tick);
  ...
  const prices = usePrices(preview.data?.preview.outputs.map(({ token }) => token) ?? []);
  return <section className="cv-editor" aria-label="Manage liquidity"><button type="button" className="cv-back" onClick={close} disabled={busy}>← All pools</button><div className="cv-editor-heading"><div><h2>{pool.name}</h2><p className="cv-muted">{coins.map((token) => token.symbol).join(" / ")}</p></div><span className="cv-badge">{CHAINS[pool.chainId].name}</span></div>
    ...
    {mode === "deposit" ? coins.map((token, i) => <AmountPanel key={i} token={token} value={amounts[i]!} change={(value) => setAmounts((old) => old.map((amount, index) => index === i ? value : amount))} balance={balanceFor(balances.data, token)} disabled={busy} label={`Deposit ${token.symbol}`} />) : <>
    ...
    {preview.loading && !preview.data && <Spinner label="Calculating liquidity…" />}
    {preview.data && <><div className="cv-output-list"><span className="cv-muted">You receive · estimated</span>{preview.data.preview.outputs.map(({ token, amount }) => <div key={tokenKey(token)}><strong>{display(amount, token.decimals)} {token.symbol}</strong><Usd value={amount} token={token} price={prices(token)} /></div>)}</div><Preview plan={preview.data} refreshing={preview.loading} /></>}
    <button type="button" className="cv-primary" disabled={busy || !account || !input || !!error || !!insufficient || !preview.data || preview.loading || !!preview.error} onClick={() => input && execute(input)}>{busy ? "Following wallet…" : insufficient ? "Insufficient balance" : !input ? "Enter an amount" : preview.loading ? "Getting quote…" : mode === "deposit" ? "Review deposit" : "Review withdrawal"}</button>
...
function Pools({ chainId, account, busy, execute, refresh }: { chainId: ChainId; account: EvmAccount | null; busy: boolean; execute: (input: Input) => void; refresh: number }) {
  ...
  if (selected) return pool.data ? <PoolEditor key={poolKey(pool.data)} pool={pool.data} close={() => setSelected(null)} {...{ account, busy, execute, refresh }} /> : <section className="cv-editor"><button type="button" className="cv-back" onClick={() => setSelected(null)}>← All pools</button>{pool.loading && <Spinner label="Verifying pool and reading your liquidity…" />}<ErrorNote error={pool.error} /></section>;
...
function App() {
  ...
  const balances = useRead(account ? `${chainId}:${account.address}` : null, (signal) => wallet.balances({ accountId: "main", chainId, tokens: [] }, { signal }), refresh);
  ...
  async function loadHistory(cursor: string | null = null) {
    const sequence = ++historyScope.current;
  ...
  useEffect(() => {
    ...
    let alive = true;
    const timer = setInterval(() => { void invoke<{ result: Result | null }>("curve_status_v1", { operationId: active.result.operationId }).then(({ result }) => { if (alive && result) setActive((old) => old && old.result.operationId === result.operationId ? { ...old, result } : old); }).catch(() => {}); }, 1800);
  ...
  return <main className="nt-app cv-app"><div className={`cv-shell ${tab === "pools" ? "cv-shell-wide" : ""}`}>
    ...
    <div className="cv-wallet"><span className="cv-wallet-dot" /><span>{account ? short(account.address) : accountRead.loading ? "Connecting to EVM Wallet…" : "Ethereum Wallet unavailable"}</span><span className="cv-right">{balances.data ? `${display(balances.data.nativeBalanceWei, 18, 5)} ETH` : "—"}</span><button type="button" className="cv-text" aria-label="Refresh wallet" onClick={() => setRefresh((value) => value + 1)} disabled={busy}>↻</button></div>
    ...
    {tab === "swap" && <Swap key={chainId} {...{ chainId, account, busy, refresh }} balances={balances.data} execute={(input) => { void execute(input); }} />}
    {tab === "pools" && <Pools key={chainId} {...{ chainId, account, busy, refresh }} execute={(input) => { void execute(input); }} />}
