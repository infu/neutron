...
import { attemptId, intentOf, latestRecord, requestId, runOperation, stateOf, trackingPausedResult, type RunOptions } from "../src/workflow.ts";
...
test("tracking interruption after approval keeps the queued Curve transaction pending", async () => {
  const f = fixture(1), controller = new AbortController();
