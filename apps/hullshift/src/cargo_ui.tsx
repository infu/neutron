...
import { CARGO_DIFFICULTIES, generateCargoPuzzle } from "./cargo_puzzles.ts";
import { HullshiftRenderer } from "./renderer.ts";
import { createInitialSnapshot } from "./simulation.ts";
import { parseShareCode } from "./share_code.ts";
...
export function HomeSurface(props: {
  ...
  onDifficulty(value: number): void; onStart(difficulty?: number, seed?: string): void;
  onHelp(): void; onSettings(): void; onOpen(id: string): void;
}) {
  const [code, setCode] = useState("");
  const [error, setError] = useState<string | null>(null);
  const profile = CARGO_DIFFICULTIES[props.difficulty]!;
  const recent = props.runs.filter((run) => run.trainingId === null && run.outcome.kind !== "victory");
  function openCode() {
    try {
      const identity = parseShareCode(code.trim());
      if (identity.generatorVersion !== "g5") throw new Error("That code is for an earlier escape puzzle. You can still resume saved escape games below.");
      setError(null);
      props.onDifficulty(identity.difficulty);
      props.onStart(identity.difficulty, formatCanonicalSeed(identity.seed));
  ...
  return <section className="hullshift-home">
    ...
    <div className="hs-home-main">
      <div className="hs-home-intro">
        ...
        <p className="hullshift-home-copy">A tiny droid. A deck full of cargo. Get every pod onto a glowing bay, one clever push at a time.</p>
      ...
      <div className="hs-home-secondary">
        ...
        <details className="hs-code"><summary>Have a puzzle code?</summary><form onSubmit={(event) => { event.preventDefault(); openCode(); }}><label className={nt.field}><span className={nt.label}>Puzzle code</span><input className={nt.input} value={code} onChange={(event) => setCode(event.target.value)} placeholder="HS1-G5-…" spellCheck={false} /></label><button className={nt.buttonSecondary} disabled={!code.trim() || props.busy}>Play this puzzle</button></form>{error ? <p role="alert">{error}</p> : null}</details>
      </div>
    </div>
    <footer className="hs-home-footer"><span>Fresh cargo. Fresh possibilities.</span><span>Made for a quiet moment.</span></footer>
  </section>;
}
...
function HomeDeck() {
  ...
  useEffect(() => {
    ...
    void generateCargoPuzzle(173n, 1).then(({ level }) => {
...
export function CargoHintMap({ run, hint }: { run: RunView; hint: HintResponse }) {
  ...
  return <svg className="hs-hint-map" viewBox={`-0.6 -0.6 ${level.width + 0.8} ${level.height + 0.8}`} aria-label="Current puzzle with row and column labels and suggested positions" role="img">
    ...
    {level.cells.map((cell, index) => <g key={index} transform={`translate(${index % level.width},${Math.floor(index / level.width)})`}><rect x={0.035} y={0.035} width={0.93} height={0.93} rx={0.08} fill={cell.terrain === "bulkhead" ? "#415773" : "#162333"} />{cell.fixture ? <rect x={0.15} y={0.15} width={0.7} height={0.7} rx={0.08} fill="none" stroke="#79e3c0" strokeWidth={0.055} /> : null}</g>)}
    {snapshot.state.objects.map((object) => <rect key={object.id} x={object.position.x + 0.22} y={object.position.y + 0.22} width={0.56} height={0.56} rx={0.09} fill="#ffb663" />)}
