...
function summary(raw: unknown): Summary {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("Invalid Aave history record.");
  const value = raw as Record<string, unknown>;
  for (const key of ["id", "summary", "phase", "revision", "created_at", "updated_at"]) if (typeof value[key] !== "string") throw new Error(`Invalid Aave history ${key}.`);
...
export function createStore(kernel: Kernel = { querySelf, updateSelf }): Store {
  return {
    async get(id) { const raw = await kernel.querySelf("aave_get_v1", [id]); return raw === null ? null : recordRow(raw); },
    async begin(input) { return recordRow(await kernel.updateSelf("aave_begin_v1", [input])); },
    async update(row, state, phase) {
      return recordRow(await kernel.updateSelf("aave_update_v1", [{ id: row.id, expected_revision: row.revision, state_json: stable(state), phase }]));
    },
    async page(cursor = null, limit = 20) {
      if (!Number.isSafeInteger(limit) || limit < 1) throw new Error("History page size must be positive.");
      const raw = await kernel.querySelf("aave_page_v1", [{ ...(cursor ? { cursor } : {}), limit: String(limit) }]) as unknown as { rows: unknown[]; next_cursor?: string };
      if (!raw || !Array.isArray(raw.rows)) throw new Error("Invalid Aave history page.");
      const rows = raw.rows.map(summary), nextCursor = raw.next_cursor ?? null;
      if (new Set(rows.map((row) => row.id)).size !== rows.length || rows.some((row) => row.id === cursor) || (nextCursor !== null && nextCursor !== rows.at(-1)?.id)) throw new Error("Aave history cursor did not advance.");
