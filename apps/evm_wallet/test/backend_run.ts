...
const tests = requested.length ? requested : ["journal_test.mo", "backend_test.mo", "erc20_decode_test.mo", "replacement_proof_test.mo", "browser_observations_test.mo"];
...
try {
  ...
  try {
    for (const test of tests) {
      if (!["journal_test.mo", "backend_test.mo", "erc20_decode_test.mo", "replacement_proof_test.mo", "browser_observations_test.mo"].includes(test)) throw new Error(`Unexpected backend test filename: ${test}`);
