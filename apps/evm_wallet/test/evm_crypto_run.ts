...
const tests = requested.length ? requested : (await fs.readdir(path.join(appRoot, "test"))).filter((name) => /^evm_crypto_.*_test\.mo$/.test(name)).sort();
const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "evm-crypto-test-"));
...
try {
  ...
  try {
    for (const test of tests) {
      if (!/^evm_crypto_.*_test\.mo$/.test(test)) throw new Error(`Unexpected cryptography test filename: ${test}`);
