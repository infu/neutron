...
const methods = { prepare: "evm_wallet_prepare_v1", reject: "evm_wallet_reject_v1", execute: "evm_wallet_execute_v1", token: "evm_wallet_asset_set_v1" };
...
async function runCase(width, form, action) {
  ...
  if (form === "token") {
    ...
    assert.match(prepared.args[0].identity.request_id, /^[0-9a-f]{32}$/);
    assert.equal(prepared.args[0].identity.caller.app_id, "evm_wallet");
    assert.equal(prepared.args[0].identity.caller.installation_uid, "12");
    ...
    const intent = prepared.args[0].intent;
    ...
    assert.equal(rejected.length, 1); assert.equal(rejected[0].args[0].identity.request_id, prepared.args[0].identity.request_id);
    assert.equal((await calls(frame, methods.execute)).length, 0, `${label}: decline executed an effect`);
    assert.equal(await frame.evaluate(() => window.__evmSandbox.toolCalls.filter((call) => !["evm_accounts_v1", "evm_operation_status_v1"].includes(call.name)).length), 1, `${label}: duplicate public tool invocation`);
  }
  assert.equal(await frame.locator(".evm-error").count(), 0, `${label}: UI error`);
...
const report = {
  passed: failure === null, scope: "Actual React/SDK in allow-scripts-only opaque-origin iframe; mocked Kernel bridge and backend replies; actual Kernel metadata encoder for captured history pagination; no real Kernel/canister/signing/on-chain proof",
