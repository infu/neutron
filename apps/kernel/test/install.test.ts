...
test("kernel generated artifacts migrate V3 to V4 and retain activation V1", async () => {
  const [
    ...
    readFile(new URL("../kernel.v0.3.39.neutron", import.meta.url)),
  ]);
  ...
  expect(manifest.version).toBe(339);
  ...
  expect(packagedArchive.manifest.version).toBe(339);
  expect(packagedArchive.packageRecord).toMatchObject({
    format: 1,
    package: { id: "kernel", version: 339 },
