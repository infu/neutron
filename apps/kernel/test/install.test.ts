...
test("kernel generated artifacts keep V3 and add isolated activation V1", async () => {
  const [
    ...
    readFile(new URL("../kernel.v0.3.10.neutron", import.meta.url)),
  ]);
  ...
  expect(manifest.version).toBe(310);
  ...
  expect(packagedManifest.version).toBe(310);
  ...
  expect(packagedArchive.manifest.version).toBe(310);
  expect(packagedArchive.packageRecord).toMatchObject({
    format: 1,
    package: { id: "kernel", version: 310 },
