...
test("kernel generated artifacts keep V3 and add isolated activation V1", async () => {
  const [
    ...
    readFile(new URL("../kernel.v0.3.24.neutron", import.meta.url)),
  ]);
  ...
  expect(manifest.version).toBe(324);
  ...
  expect(packagedArchive.manifest.version).toBe(324);
  expect(packagedArchive.packageRecord).toMatchObject({
    format: 1,
    package: { id: "kernel", version: 324 },
