...
test("kernel generated artifacts restore V4 and activation V1", async () => {
  const [
    ...
    readFile(new URL("../kernel.v0.3.49.neutron", import.meta.url)),
  ]);
  ...
  expect(manifest.version).toBe(349);
  ...
  expect(packagedManifest.version).toBe(349);
  ...
  expect(packagedArchive.manifest.version).toBe(349);
  expect(packagedArchive.packageRecord).toMatchObject({
    format: 1,
    package: { id: "kernel", version: 349 },
...
test("release 349 preserves deployed Kernel 336/343/344/346/347/348 lineage and the complete 348 backend", async () => {
  const currentFiles = unpackNeutronPackage(
    await readFile(new URL("../kernel.v0.3.49.neutron", import.meta.url)),
  );
  ...
  expect(current.version).toBe(349);
  ...
  for (const predecessor of [
    ...
    if (predecessor.version === 346 || predecessor.version === 347 || predecessor.version === 348) {
