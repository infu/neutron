...
describe("Kernel v322 NPL package metadata", () => {
  ...
  test("fails closed on wrong release identity, license, notice, or memory", async () => {
    ...
    ).toThrow("restricted to Kernel version 322");
    expect(() =>
      buildKernelPackageMetadata({
        ...fixture,
  ...
  test("generates a deterministic inventory of exact installed Kernel artifacts", async () => {
    ...
    await installKernelInstalledArtifactInventory(root, 322);
    ...
    expect(parsed.package).toEqual({ id: "kernel", version: 322 });
