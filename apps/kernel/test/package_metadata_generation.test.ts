...
describe("Kernel v354 NPL package metadata", () => {
  ...
  test("fails closed on wrong release identity, license, notice, or memory", async () => {
    ...
    ).toThrow("restricted to Kernel version 354");
    expect(() =>
      buildKernelPackageMetadata({
        ...fixture,
  ...
  test("generates a deterministic inventory of exact installed Kernel artifacts", async () => {
    ...
    await installKernelInstalledArtifactInventory(root, 354);
    ...
    expect(parsed.package).toEqual({ id: "kernel", version: 354 });
  ...
  test("current workspace review includes every required changed source", async () => {
    ...
    for (const required of [
      ...
      "packages/neutron-tools/src/certified_asset.ts",
      "packages/neutron-tools/test/certified_asset.test.ts",
