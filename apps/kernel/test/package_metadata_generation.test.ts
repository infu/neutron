...
describe("Kernel v328 NPL package metadata", () => {
  ...
  test("fails closed on wrong release identity, license, notice, or memory", async () => {
    ...
    ).toThrow("restricted to Kernel version 328");
    expect(() =>
      buildKernelPackageMetadata({
        ...fixture,
  ...
  test("generates a deterministic inventory of exact installed Kernel artifacts", async () => {
    ...
    await installKernelInstalledArtifactInventory(root, 328);
    ...
    expect(parsed.package).toEqual({ id: "kernel", version: 328 });
