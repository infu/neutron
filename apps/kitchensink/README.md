...
| Wallet funding provider | Ask Wallet to transfer 0.01 ICP directly to Neutrinite governance or grant that governance canister a five-minute transfer-from allowance, with one Wallet-owned approval per action. | Implemented; Kitchen Sink never receives ledger authority and cannot consume an allowance whose spender is Neutrinite governance. |
...
`kitchensink.v0.3.6.neutron`. The backend selects only the exact capability leaf
