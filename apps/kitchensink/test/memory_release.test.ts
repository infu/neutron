...
test("Kitchen Sink 0.3.14 keeps the exact production 0.3.10 v1 memory root across skipped releases", async () => {
  ...
  await assertManagedMemoryCodeOnlyRelease({
    ...
    candidateVersion: 314,
  });
});

test("Kitchen Sink 0.3.14 keeps the exact production 0.3.11 v1 memory root", async () => {
  await assertManagedMemoryCodeOnlyRelease({
    appId: "kitchensink",
    memoryId: "kitchensink",
    productionArchive: new URL("../kitchensink.v0.3.11.neutron", import.meta.url),
    candidateArchive: new URL("../kitchensink.v0.3.14.neutron", import.meta.url),
    lock: new URL("../neutron.lock.json", import.meta.url),
    production: {
      version: 311,
      bytes: 430_950,
