...
const archive = new URL("../hyperliquid.v0.1.2.neutron", import.meta.url);
const predecessors = [
...
test("release installs its complete UI, resident service and private journal", async () => {
  ...
  expect(sourceManifest).toMatchObject({ id: "hyperliquid", version: 102, update_source: "233tv-xiaaa-aaaay-aacta-cai" });
  const files = unpackNeutronPackage(await readFile(archive));
  const prepared = preparePackageInstall(files);
  expect(prepared.manifest).toMatchObject({ id: "hyperliquid", version: 102 });
...
test("release contains the shared use license and the exact offered source artifact", async () => {
  ...
  const snapshot = decodeNeutronAppSourceSnapshot(new Uint8Array(gunzipSync(bytes)), { id: "hyperliquid", version: 102 });
