// V101 pristine-install schema: keep this file immutable after release.
// It deliberately has no V2 -> V3 migration because pre-V101 AcceptedLike
// rows cannot be proven to have passed browser certificate verification.
// V3 also adds bounded local delivery-use accounting for automatic Follow
// renewal and direct indexes for authored nonces and received feed slots;
// pristine installs start those sparse maps empty.
// Package imports are allowed; relative imports are forbidden so app-local
// types cannot drift.
//
...
module {
    ...
    public type Mem = {
        ...
        // Sparse, local-only count of first durable verified promotions for
        // the current subscription since its last acknowledged paid Follow.
        // Zero entries are omitted and values are saturated by the
        // relationship service, so cardinality is bounded by active
        // Following capacity.
        locally_verified_delivery_counts : Map.Map<Principal, Nat16>;
        ...
        // post nonce -> post key. Publication idempotency must never scan the
        // authored-post store.
        authored_post_by_nonce : Map.Map<StableKey, StableKey>;
        ...
        // (claimed author, claimed post id) -> at most the fixed small
        // candidate-slot cap. This bounds conflict quarantine and withdrawal
        // visibility work independently of the global candidate quota.
        feed_candidates_by_claimed_slot :
            Map.Map<StableKey, [StableKey]>;
        ...
        // (verified author, verified post id) -> canonical feed key.
        verified_feed_by_post_slot : Map.Map<StableKey, StableKey>;
    ...
    public func init() : Mem {
        {
            ...
            locally_verified_delivery_counts =
                Map.empty<Principal, Nat16>();
            ...
            authored_post_by_nonce = Map.empty<StableKey, StableKey>();
