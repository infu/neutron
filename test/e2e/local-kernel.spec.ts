...
test("URL package source reaches the existing install review", async ({
  ...
  const packageUrl =
    "https://packages.test/vetkeys_fixture.v0.1.0.neutron";
  await page.route(packageUrl, async (route) => {
    await route.fulfill({
      contentType: "application/octet-stream",
      headers: { "access-control-allow-origin": "*" },
      path: "apps/vetkeys_fixture_test/vetkeys_fixture.v0.1.0.neutron",
    });
  });
...
async function installHelloFromBrowser(page: Page): Promise<void> {
  ...
  await fileChooser.setFiles("apps/hello/hello.v0.2.0.neutron");
