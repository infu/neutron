...
export const TEST_CANDIDATE_KERNEL_VERSION = 309;
...
/**
 * Decode and compile the actual packed v0.3.9 candidate. This never invokes
...
async function compileLegacyKernelUpgradeCandidate(
  ...
  if (candidateKernel.packageRecord === undefined) {
    throw new Error("The v0.3.9 candidate package record was not verified");
  }
...
function assertFinalCandidatePackageRecord(
  ...
  if (
    ...
    throw new Error(
      "The v0.3.9 candidate must carry the reviewed NPL source record",
    );
  }
}
...
function assertCandidateManifest(
  ...
  if (
    ...
    throw new Error(
      "The candidate must be Kernel v0.3.9 in format 3 with the production update source",
    );
  }
}
...
export function assertLegacyUpgradeCompileInvariants(
  ...
  for (const expected of fixture.identity.memory) {
    ...
    if (
      ...
      throw new Error(
        `The v0.3.9 candidate changed released memory ${expected.id} v${expected.version}`,
      );
    }
  }
}
...
async function testCandidateArchive(
  ...
  const offeredSource = encoder.encode(
    "synthetic v0.3.9 corresponding-source fixture\n",
  );
