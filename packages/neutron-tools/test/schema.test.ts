...
test("package features are closed, canonical, and unavailable to Kernel packages", () => {
  const app: NeutronManifest = {
    format: 3,
    id: "good_app",
    name: "Good App",
    version: 100,
  ...
  expect(normalizeManifestPackageFeatures(app)).toEqual([
    NEUTRON_PACKAGE_ARCHIVE_ONLY_FEATURE,
  ]);
  expect(() => assertNeutronManifest(app, "source")).toThrow(
    /package_features is generated by packaging/,
  );

  for (const package_features of [
    [],
    ["unknown-feature"],
    [
      NEUTRON_PACKAGE_ARCHIVE_ONLY_FEATURE,
    ...
    expect(
      validateManifest({ ...app, package_features }).errors.length,
    ).toBeGreaterThan(0);
  }

  const kernel = { ...app, id: "kernel", name: "Kernel" };
  expect(validateManifest(kernel).errors.length).toBeGreaterThan(0);
  expect(() => assertNeutronManifest(kernel, "package")).toThrow(
    /Invalid package_features declaration/,
  );
});
